...
create table public.category_attachments (
  ...
  constraint category_attachment_shape check (
...
create index category_attachments_category_idx
on public.category_attachments(category_id, created_at);

alter table public.category_attachments enable row level security;

create policy "members view category attachments"
on public.category_attachments for select
using (public.can_access_category(category_id));

create policy "owners create category attachments"
on public.category_attachments for insert
with check (public.is_app_owner() and created_by = auth.uid());

create policy "owners delete category attachments"
...
create or replace function public.category_id_from_storage_path(object_name text)
...
create policy "members read category files"
on storage.objects for select
using (
  bucket_id = 'category-attachments'
  and public.can_access_category(public.category_id_from_storage_path(name))
);

create policy "owners delete category files"
on storage.objects for delete
using (bucket_id = 'category-attachments' and public.is_app_owner());

alter publication supabase_realtime add table public.category_attachments;
