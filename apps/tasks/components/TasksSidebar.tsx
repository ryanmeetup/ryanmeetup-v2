      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-black/10 bg-white p-4 transition-transform dark:border-white/10 dark:bg-black lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="shrink-0 space-y-2 border-t border-black/10 pt-4 dark:border-white/10">
          ...
          <div className="flex items-center gap-3 px-2 py-2">
            {demoMode ? <div className="flex min-w-0 flex-1 items-center gap-3"><Avatar name={name} src={data.currentProfile.avatar_url} /><ProfileSummary name={name} demo /></div> : <Link href="/profile" className="-ml-2 flex min-w-0 flex-1 items-center gap-3 rounded-lg px-2 py-2 transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/30 dark:hover:bg-white/10 dark:focus-visible:ring-white/40"><Avatar name={name} src={data.currentProfile.avatar_url} /><ProfileSummary name={name} /></Link>}
