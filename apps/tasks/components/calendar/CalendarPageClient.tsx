              ...
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-end justify-end gap-2">
                  {(googleCanManage ||
                    ...
                    >
                      <span
                        className={`h-2 w-2 rounded-full ${googleConnection.connected ? "bg-emerald-500" : "bg-black/30 dark:bg-white/30"}`}
                        aria-hidden
                      />
                  ...
                  {days.map((date) => {
                    ...
                    return (
                      ...
                      >
                        <button
                          type="button"
                          onClick={() => openNew("important", date)}
          ...
          <div className="space-y-5">
              ...
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm">
                {previewing ? (
                  ...
                  <>
                    This was logged by{" "}
                    {profileDisplayName(
                      profiles.get(editingEvent?.created_by ?? ""),
                    )}
                    . Only that teammate, the person who is away, or an app
                    owner can change it.
                  </>
                )}
              </div>
              ...
              (canEdit ? (
                ...
                <div>
                  ...
                  <p className="mt-2 flex items-center gap-2 text-sm">
                    <Avatar
                      name={profileDisplayName(profiles.get(draft.profileId))}
                      src={profiles.get(draft.profileId)?.avatar_url}
                      size="sm"
                    />
                    {profileDisplayName(profiles.get(draft.profileId))}
                  </p>
                </div>
              ))}
            <Input
            ...
            <label className="flex items-center gap-3 text-sm font-medium">
              <input
                ...
                disabled={!canEdit || saving}
                onChange={(event) =>
                  updateDraft("allDay", event.target.checked)
                }
            ...
            {!draft.allDay && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-black/55 dark:text-white/55">
                  Saved in {workspaceTimeZoneLabel(draft.startDate, "long")},
                  and shown that way to every teammate.
                </p>
              </div>
            )}
