...
/**
 * Adds a pasted checklist in one transaction.
 *
 * Ids are generated by the caller so the optimistic rows already on screen can
 * be swapped for the saved ones rather than duplicated alongside them.
 */
export function createSubtasks(input: {
  ...
  return expectRow(
    mutate<{ subtasks?: Subtask[]; activity?: TaskActivity }>(
      DETAILS,
      json("POST", {
        kind: "subtasks",
        taskId: input.taskId,
    ...
    ["subtasks", "activity"],
    "The checklist items could not be added.",
  );
}
...
export function deleteSubtask(id: string) {
  return mutate<{ id: string; activity?: TaskActivity }>(
...
export function createComment(input: {
  ...
  return expectRow(
    mutate<{ comment?: TaskComment; activity?: TaskActivity }>(
      DETAILS,
      json("POST", {
        kind: "comment",
        taskId: input.taskId,
...
export function deleteAttachment(id: string) {
  return mutate<{ deleted: boolean; activity?: TaskActivity }>(
