...
The Vercel worker runs at 13:00 UTC Monday through Friday. Dates are classified
in `America/New_York`, the workspace's current operating timezone. Resend uses
an idempotency key made from the recipient and digest date, so retries on the
same day do not create duplicate messages.
