...
describe("task board relation selectors", () => {
  it("indexes every category without duplicating relations", () => {
    const rows = [
      { task_id: "task", category_id: "one" }, { task_id: "task", category_id: "two" },
  ...
  it("merges legacy primary assignees with relation rows", () => {
    ...
    const rows = [{ task_id: "task", profile_id: "secondary" }] as TaskAssignee[];
    expect([...indexTaskAssignees(tasks, rows).get("task")!]).toEqual(["primary", "secondary"]);
  });
});
