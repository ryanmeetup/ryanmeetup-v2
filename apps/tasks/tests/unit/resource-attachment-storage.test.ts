...
import { attachmentObjectPath, removeAttachmentObject } from "@/lib/server/resource-attachment-storage";
...
describe("resource attachment storage cleanup", () => {
  ...
  it("creates resource-scoped paths with safe file names", () => {
    expect(attachmentObjectPath("project", "attachment", "launch brief (final).pdf"))
  ...
  it("returns cleanup failures so callers can report or log them", async () => {
    ...
    await expect(removeAttachmentObject("project/file.pdf", "project-attachments")).resolves.toBe(failure);
